...
interface Props {
  ...
  solutions: IndustrySolutionData[];
  accent?: IndustryAccent;
}

export function IndustrySolutionsSection({
  id = "industry-solutions",
  chapter = "03 / INDUSTRY SOLUTIONS",
  heading = "Capabilities Matrix",
  subtext,
  industryCategory = "Industry Systems",
  solutions,
  accent = "emerald",
}: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1560px] px-6 sm:px-10 lg:px-16 xl:px-20">
        <div className="bg-white/80 backdrop-blur-2xl rounded-3xl border border-white/90 shadow-[0_16px_50px_0_rgba(0,0,0,0.06)] p-6 sm:p-10 lg:p-12">
                ...
                {solutions.map((solution, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "group relative flex items-center py-3.5 px-4 sm:px-5 rounded-xl text-left transition-all duration-200 focus-visible:outline-none focus-visible:ring-2",
                        a.focusRing,
                        isActive
                    ...
                    >
                      {isActive && (
                        <motion.div
                          layoutId={`activeIndustrySolutionBar-${id}`}
                  ...
                  <div className="relative w-full aspect-[16/7] sm:aspect-[21/8] overflow-hidden rounded-2xl bg-neutral-900 border border-neutral-200/80 shadow-xs">
                    ...
                    <div className="absolute bottom-4 left-6 right-6 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="flex items-center gap-2 font-mono text-xs font-semibold text-white uppercase tracking-widest">
                        <span className={cn("font-bold", a.statText)}>{active.title}</span>
                        <span>·</span>
                        <span className="text-neutral-300">{industryCategory}</span>
                      </div>
                    ...
                    <div className="flex flex-col gap-4">
                      {left.map((item) => (
                        <Link
                          ...
                          className={cn(
                            "group inline-flex items-baseline gap-2 text-sm sm:text-base font-normal text-neutral-800 transition-colors focus-visible:outline-none focus-visible:ring-2 rounded",
                            a.focusRing,
                            `hover:${a.text}`
                          )}
                        >
