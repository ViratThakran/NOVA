...
export function SharedInActionSection({ id, chapter, heading, subtext, stories, accent }: Props) {
    ...
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1560px] px-6 sm:px-10 lg:px-16 xl:px-20">
        <div className="flex flex-col gap-3 max-w-2xl mb-16 sm:mb-20">
          ...
          <p className="text-base sm:text-lg text-neutral-400 font-normal leading-relaxed mt-2">{subtext}</p>
        </div>
          ...
          {stories.map((story, index) => (
            ...
            >
              <div className={`lg:col-span-5 ${index % 2 === 1 ? "[direction:ltr]" : ""}`}>
                <div className="relative w-full aspect-[16/10] overflow-hidden rounded-2xl border border-white/10 bg-neutral-900">
                  <Image
                    src={story.image}
                    alt={story.title}
                    fill
              ...
              <div className={`lg:col-span-7 flex flex-col gap-5 ${index % 2 === 1 ? "[direction:ltr]" : ""}`}>
                ...
                <p className="text-base sm:text-lg text-neutral-400 leading-relaxed">{story.synopsis}</p>
                <div className="pt-4 border-t border-white/10">
                  <span className={cn("text-xs font-mono tracking-widest uppercase", a.statText)}>
                    {story.stat}
                  </span>
                </div>
              </div>
