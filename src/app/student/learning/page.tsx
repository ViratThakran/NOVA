...
export default async function StudentLearningPage() {
  ...
  if (!journey) return <div className="flex flex-col gap-8"><PageHeader title="Learning" description="Your learning activity and progress." /><EmptyState title="Your AI internship journey will appear here" description="Once an internship application is accepted, NOVA's AI Mentor will build your first task." /></div>;

  const { data: tasks } = await auth.supabase.from("internship_ai_tasks").select("id, sequence_no, title, objective, instructions, deliverables, acceptance_criteria, estimated_hours, status, attempt_count").eq("journey_id", journey.id).order("sequence_no", { ascending: true });
  const { data: reviews } = await auth.supabase.from("internship_ai_reviews").select("id, task_id, verdict, score, summary, strengths, improvements, next_step, created_at").eq("student_id", auth.user.id).order("created_at", { ascending: false });
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {journey.last_error && journey.status === "failed" && <div className="rounded-2xl border border-error/30 bg-error/5 p-5 text-small">Your mentor hit a temporary processing issue. Your internship state is saved and can be retried by the NOVA team.</div>}

      {currentTask ? (
        <section className="rounded-2xl border border-border bg-surface p-6 md:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-small text-text-muted">Task {currentTask.sequence_no}</p><h2 className="mt-1 text-2xl font-semibold">{currentTask.title}</h2><p className="mt-3 max-w-3xl text-text-muted">{currentTask.objective}</p></div><span className="rounded-full bg-surface-muted px-3 py-1 text-small">~{currentTask.estimated_hours}h</span></div>
