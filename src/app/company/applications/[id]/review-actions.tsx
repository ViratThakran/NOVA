...
import { markCompanyUnderReviewAction } from "../../actions";
import { reviewCompanyApplicationWithMentorAction } from "./mentor-review-action";
...
export function CompanyReviewActionButtons({ applicationId, canMarkUnderReview, canReview }: { applicationId: string; canMarkUnderReview: boolean; canReview: boolean }) {
  const router = useRouter();
  const [markState, markFormAction, markPending] = useActionState(markCompanyUnderReviewAction, initialCompanyActionState);
  const [reviewState, reviewFormAction, reviewPending] = useActionState(reviewCompanyApplicationWithMentorAction, initialCompanyActionState);
    ...
    <div className="flex flex-col gap-4">
      ...
      <Modal open={confirming !== null} onClose={() => setConfirming(null)} title={confirming === "accepted" ? "Accept this application?" : "Reject this application?"} description={confirming === "accepted" ? "This creates an enrollment, starts the AI Mentor journey, and notifies the student." : "This notifies the student that their application was not selected."}>
        <form action={reviewFormAction} className="flex flex-col gap-4">
          <input type="hidden" name="application_id" value={applicationId} /><input type="hidden" name="status" value={confirming ?? ""} />
