...
import { updateInternshipStatusAction } from "../../actions";
...
export function StatusControl({ internshipId, currentStatus }: { internshipId: string; currentStatus: InternshipStatus }) {
  const router = useRouter();
  const [state, formAction, pending] = useActionState(updateInternshipStatusAction, initialAdminActionState);
  ...
  return (
    <div className="flex flex-col gap-2">
      <Select
        ...
        value={pendingStatus}
        onChange={(event) => setPendingStatus(event.target.value as InternshipStatus)}
      >
