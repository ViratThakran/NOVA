...
import { markUnderReviewAction, reviewApplicationAction } from "../../actions";
...
export function ReviewActionButtons({
  ...
  const [reviewState, reviewFormAction, reviewPending] = useActionState(reviewApplicationAction, initialAdminActionState);
  const [confirming, setConfirming] = useState<ReviewDecision | null>(null);

  // Both actions wrap RPCs that change the application's status — on
  // success, refresh so this server component re-fetches and the button
  // set updates to match the new state, rather than duplicating that logic.
  useEffect(() => {
